package com.zane49er.magus.features.gui.writing;
...
public class GUIBookLvl1 extends GuiScreen {
	...
	ResourceLocation texture = new ResourceLocation(References.MOD_ID, "textures/gui/RiftTomeGui.png");
	int GUIWidth = 170;
	int GUIHeight = 220;
	int left;
	int top;
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawString(this.fontRenderer, pageName, left + 10, top + 10, 0x00FFAA);
		...
		for (int i = 0; i < bItems.size(); i++) {
			...
			if (b.hovering) {
				...
				// j.add(b.name);
				GlStateManager.pushMatrix();
				drawHoveringText(b.text, mouseX, mouseY, this.fontRenderer);
				GlStateManager.popMatrix();
			}
			...
			int Ty = (int) (b.y) + yScroll;
			{
				for (int j = 0; j < b.text.size(); j++) {
					for (int k = 0; k < b.text.get(j).length(); k++) {
						...
						GlStateManager.scale(b.scale, b.scale, b.scale);
						if (cx > left && cy > top && cx < left + GUIWidth && cy < top + GUIHeight) drawString(this.fontRenderer, String.valueOf(b.text.get(j).charAt(k)), 0, 0, 0xFFFFFF);
						GlStateManager.popMatrix();
					}
				}
			}
		...
		while (pageData.has("line" + i)) {
			...
			while (segs.has("s" + j)) {
				...
				for (int k = 0; k < dat.get("len").getAsInt(); k++) {
					...
					b.scale = scale;
					b.texture = new ResourceLocation(References.MOD_ID, "textures/gui/RiftTomeGui.png");
					b.u = 170;
